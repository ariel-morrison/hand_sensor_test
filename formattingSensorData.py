...
def get_beri_for_plotting(working_dir, beri_exists, beri_files):
...
def plot_results(Fs, pref_dpi, EDA_data_df, EDA_data_df2, output_dir, separate_baseline, continuous_baseline, beri_exists, EDA_by_sensor):
...
# If baseline method = entire record (averaged over entire semester, day, etc):
    ...
    fig1, ax = pl.subplots( nrows=1, ncols=1 )
    if beri_exists == True:
        pl.scatter(grades_merged_beri['Final Grade'], grades_merged_beri['% diff'], c = 'k', marker='o', s=12)
    else:
    ...
    if beri_exists == True:
        pl.scatter(grades_merged_beri['Final Exam'], grades_merged_beri['% diff'], c = 'b', marker='o', s=12)
    else:
    ...
    if beri_exists == True:
        statistics_output = percent_diff_means_no_outliers, percent_diff_stddev_no_outliers, percent_diff_stderr_no_outliers, \
                            percent_diff_means_no_outliers_beri, \
    ...
    if beri_exists == True:
        ...
        # pl.scatter(range(0,len(beri_data['class_subject_code'])), beri_data['total_eng'], c = 'k', marker='o', s=3, label='# engaged')
        # pl.scatter(range(0,len(beri_data['class_subject_code'])), beri_data['total_diseng'], c = 'b', marker='v', s=3, label="# disengaged")
...
def save_output_csv(statistics_output, output_dir, keywords, activity_stats, beri_exists):
    ...
    if beri_exists == True:
        ...
        out_df.to_csv(os.path.join(output_dir, filename), index=False, header=['Activity', 'Mean % diff, no outliers', 'Std. dev. of mean % diff, no outliers', \
                    'Std. err. of mean % diff, no outliers', 'Mean % diff, BERI no outliers', 'Std. dev. of mean % diff, BERI no outliers',\
